...
import { createSymptomLogSchema } from "@/lib/validations/symptom-log";
...
// ── GET /api/patients/[id]/symptom-logs ─────────────────────────────
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ symptomLogs: data });
  } catch {
    return jsonError("Internal server error", 500);
  }
}

// ── POST /api/patients/[id]/symptom-logs ────────────────────────────
...
export async function POST(
  ...
  try {
    ...
    auditLog({
      ...
      resourceId: data.id,
      detail: { patient_id: patientId, symptom_name: parsed.data.symptom_name },
      request,
    });

    return NextResponse.json({ symptomLog: data }, { status: 201 });
  } catch {
    return jsonError("Internal server error", 500);
  }
}
