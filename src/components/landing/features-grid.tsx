        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((f, i) => (
            <ScrollReveal key={f.title} delay={i * 100}>
              <div className="h-full p-6 rounded-[var(--radius-lg)] bg-[var(--color-surface-secondary)] border border-[var(--color-border-light)] hover:border-[var(--color-brand-300)] hover:shadow-lg transition-all">
                <div className="space-y-4">
                  <div className="w-11 h-11 rounded-lg bg-[var(--color-brand-50)] flex items-center justify-center">
                    {f.icon}
                  </div>
